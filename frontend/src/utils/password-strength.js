...
export const strengthColor = count => {
	if (count < 2) return { label: '위험', color: 'error.main' };
	if (count < 3) return { label: '약함', color: 'warning.main' };
	if (count < 4) return { label: '보통', color: 'warning.dark' };
	if (count < 5) return { label: '좋음', color: 'success.main' };
	if (count < 6) return { label: '강력', color: 'success.dark' };
	return { label: '위험', color: 'error.main' };
};
